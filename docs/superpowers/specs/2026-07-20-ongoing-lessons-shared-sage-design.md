...
- Remove the “Private lessons / Ukulele & guitar / Kihei / Wailea / From $35” trust strip entirely; the hero should flow directly into the lesson pathways.
...
- Scope production changes to `src/components/weekly/WeeklyJourneySections.tsx` and `src/components/weekly/WeeklyJourneySections.css`.
- Extend `test/weekly-rhythm-faithful.test.mjs` to guard use of the shared sage token, absence of the obsolete local sage token, and absence of the removed trust strip.
